...
codeArgs = ("argcount",) + (("posonlyargcount",) if sys.version_info >= (3, 8) else ()) + ("kwonlyargcount", "nlocals", "stacksize", "flags", "codestring", "constants", "names", "varnames", "filename", "name", "firstlineno", "lnotab", "freevars", "cellvars")
...
def bindVarsToFunc(f: FunctionType, inlines: dict = None, returnInfo=False):
	...
	#print(scope)
	patch = {opcVarCollMapping[opc]: tuple(symbolsIndexRemap.values()) for opc, symbolsIndexRemap in scope.items()}
